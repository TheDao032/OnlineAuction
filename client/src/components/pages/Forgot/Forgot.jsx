...
function Forgot(props) {
  ...
  const handleOnSubmit = async function (data) {
    ...
    try {
      ...
      const res = await axios.post('https://onlineauctionserver.herokuapp.com/api/authentication/forgot-password', {
      ...
      {loadingState.loading ? <Loading /> :
        <div className='forgot'>
          <div className='forgot__container'>
            ...
            <form className="forgot__form" onSubmit={form.handleSubmit(handleOnSubmit)}>
              <InputField form={form} name='accEmail' label='Email' labelClass='form__group-label' />
              <input type='submit' className='forgot__button' value='Xác nhận' />
            </form>
          </div>
        </div>
      }
