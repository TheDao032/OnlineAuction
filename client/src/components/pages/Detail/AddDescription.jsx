...
function AddDescription({ onCancel }) {
  ...
  async function handleSubmit() {
    ...
    if (prodDescription === '') {
      ...
      try {
        ...
        const res = await axios.post('https://onlineauctionserver.herokuapp.com/api/seller/update-description', data, {
          headers: {
  ...
  return <div className='detail__description-add'>
    <p>Thêm mô tả</p>
    <EditorField onEditorStateChange={getDetailEditor} />
    <div className='detail__description-action'>
      <button className='detail__description-action--submit' onClick={handleSubmit}>Thêm mô tả</button>
      <button className='detail__description-action--cancel' onClick={handleCancel}>Hủy</button>
    </div>
  </div>;
}
